/*
 * decomp/src/netplay/menus/mnvsoptions_netmenu.c — netmenu fork of mn/mnvsmode/mnvsoptions.c.
 * Offline classic VS Options backs to nSCKindVSOfflineClassic (mnvsoffline); hub backs to nSCKindVSMode.
 * SSB64_NETMENU only (stock TU excluded in cmake/Ssb64NetmenuSources.cmake).
 */
...
void mnVSOptionsFuncRun(GObj *gobj)
{
    ...
    if (sMNVSOptionsTotalTimeTics >= 10)
    {
        ...
        if (scSubsysControllerGetPlayerTapButtons(B_BUTTON) != FALSE)
        {
            ...
            return_scene = syNetmenuVsMenuReturnScene();
            gSCManagerSceneData.scene_prev = gSCManagerSceneData.scene_curr;
            gSCManagerSceneData.scene_curr = return_scene;
