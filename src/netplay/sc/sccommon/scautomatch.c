...
#define MN_AM_BIND_DEFAULT "0.0.0.0:0"
...
#if defined(SSB64_NETPLAY_ICE)
/* Faster match poll while libjuice negotiates (trickle drain via GET /v1/match). */
#define MN_AM_ICE_POLL_INTERVAL 2U
#endif
...
static u64 sMnAMBindSrflxLogMs;
#endif

static const char *mnVSNetAutomatchAMStateName(MnVSNetAutomatchAMState state)
{
	switch (state)
	{
	case MN_AM_IDLE:
...
static void mnVSNetAutomatchAMSetState(MnVSNetAutomatchAMState state, const char *why)
{
	MnVSNetAutomatchAMState prev;
...
static void mnVSNetAutomatchAMHostFromHostport(const char *hostport, char *host_out, u32 host_cap)
{
	const char *colon;

	if ((host_out == NULL) || (host_cap == 0U))
	{
		return;
	}
	host_out[0] = '\0';
	...
	{
		snprintf(host_out, host_cap, "%.*s", (int)(colon - hostport), hostport);
	}
}

static sb32 mnVSNetAutomatchAMQueueWanHostIsRfc1918(const char *hostport)
{
	char host[64];

	mnVSNetAutomatchAMHostFromHostport(hostport, host, sizeof(host));
	return mmLanIpv4StringIsRfc1918(host);
}

/* Prefer STUN srflx; fall back to RFC1918 host bind (not carrier/public relay IP). */
static sb32 mnVSNetAutomatchAMResolveQueueWanEndpoint(char *out, u32 out_cap)
{
	...
	{
		if ((out[0] != '\0') && (strncmp(out, "0.0.0.0", 7) != 0) && (mnVSNetAutomatchAMQueueWanHostIsRfc1918(out) != FALSE))
		{
			return TRUE;
		}
		out[0] = '\0';
	...
	if ((ice_lan != NULL) && (ice_lan[0] != '\0') && (mnVSNetAutomatchAMQueueWanHostIsRfc1918(ice_lan) != FALSE))
	{
		snprintf(out, out_cap, "%s", ice_lan);
		return TRUE;
	}
	...
	if ((sMnAMBindSpec[0] != '\0') && (strncmp(sMnAMBindSpec, "0.0.0.0", 7) != 0))
	{
		snprintf(out, out_cap, "%s", sMnAMBindSpec);
		return TRUE;
	}
	{
		const char *lan_ep;

		lan_ep = mnVSNetAutomatchAMLanPtr(NULL);
		if ((lan_ep != NULL) && (lan_ep[0] != '\0'))
		{
			snprintf(out, out_cap, "%s", lan_ep);
			return TRUE;
		}
	}
	return FALSE;
...
static void mnVSNetAutomatchAMMaybeEnqueueMatchPoll(u32 base_interval, sb32 trickle_only)
{
	if (sMnAMTicket[0] == '\0')
	{
		return;
...
static void mnVSNetAutomatchAMResetSearchState(void)
{
	mnVSNetAutomatchAMSetState(MN_AM_IDLE, "reset");
	sMnAMTicket[0] = '\0';
...
static void mnVSNetAutomatchAMResetTransport(void)
{
	syNetPeerClearAutomatchBootstrapContext();
#if defined(SSB64_NETPLAY_ICE)
	sMnAMBindSrflxLogMs = 0U;
...
void mnVSNetAutomatchAMReset(void)
{
	mnVSNetAutomatchAMResetSearchState();
	mnVSNetAutomatchAMResetTransport();
}
...
static void mnVSNetAutomatchAMResolvePeerLan(char *peer_lan_out, u32 cap, const MmMatchResult *mr)
{
	...
	if ((peer_lan_out == NULL) || (cap == 0U) || (mr == NULL))
	{
		return;
	}
	peer_lan_out[0] = '\0';
	...
	{
		snprintf(peer_lan_out, cap, "%s", mr->peer_lan_hostport);
		return;
	}
	env = getenv("SSB64_MATCHMAKING_PEER_LAN");
	if ((env != NULL) && (env[0] != '\0'))
	{
		snprintf(peer_lan_out, cap, "%s", env);
		port_log("SSB64 Automatch: using SSB64_MATCHMAKING_PEER_LAN=%s (server peer_lan missing)\n", peer_lan_out);
	}
}

static void mnVSNetAutomatchAMErrEx(const char *reason)
{
	const char *why;
...
{
	mnVSNetAutomatchAMErrEx("connection failed");
}
					       ...
					       const char *turn_permission_hp, sb32 use_turn_outbound, sb32 keep_turn_relay)
{
	...
	if (syNetPeerConfigureUdpForAutomatch(bind, peer_hp, mr->session_id, mr->you_are_host, 2U, TRUE) == FALSE)
	{
...
static void mnVSNetAutomatchAMTryPendingLanBootstrap(void)
{
	const char *bind;

	if (sMnAMPendingLanBootstrap == FALSE)
	{
		return;
	}
	bind = (sMnAMBindSpec[0] != '\0') ? sMnAMBindSpec : MN_AM_BIND_DEFAULT;
	if (mnVSNetAutomatchAMTryBootstrap(&sMnAMPendingMatch, bind, sMnAMPendingMatch.peer_lan_hostport) != FALSE)
	{
		port_log("SSB64 NetPeer automatch: reachability candidate=lan ok peer=%s\n",
		         sMnAMPendingMatch.peer_lan_hostport);
		sMnAMStagingP2PReady = TRUE;
		sMnAMPendingLanBootstrap = FALSE;
		mnVSNetAutomatchAMSetState(MN_AM_ENTER, "lan bootstrap ok");
		return;
	}
	sMnAMPendingLanBootstrap = FALSE;
	if (syNetPeerAutomatchBootstrapWasAborted() != FALSE)
	...
	{
		mnVSNetAutomatchAMErrEx("lan bootstrap failed");
	}
}
...
static void mnVSNetAutomatchAMEnterVs(const MmMatchResult *mr)
{
	...
	const char *queued_wan;
	char refreshed_wan[144];
	char peer_lan[144];
	...
	if (sMnAMPublicEndpoint[0] != '\0')
	{
		snprintf(refreshed_wan, sizeof(refreshed_wan), "%s", sMnAMPublicEndpoint);
	}
	if ((queued_wan[0] != '\0') && (refreshed_wan[0] != '\0') && (strcmp(queued_wan, refreshed_wan) != 0))
	{
		port_log("SSB64 Automatch: reflexive endpoint changed queue=%s bootstrap=%s\n", queued_wan, refreshed_wan);
	}
	...
	memcpy(&mr_work, mr, sizeof(mr_work));
	mnVSNetAutomatchAMResolvePeerLan(peer_lan, (u32)sizeof(peer_lan), mr);
	if (peer_lan[0] != '\0')
	{
	...
	if (try_lan_first != FALSE)
	{
		if (peer_lan[0] != '\0')
		{
			if (mnVSNetAutomatchAMTryBootstrap(&mr_work, bind, peer_lan) != FALSE)
			{
				port_log("SSB64 NetPeer automatch: reachability candidate=lan ok peer=%s\n", peer_lan);
				sMnAMStagingP2PReady = TRUE;
				return;
	...
	if ((force_lan_first == FALSE) && (peer_lan[0] != '\0'))
	{
		if (same_wan == FALSE)
		{
			port_log(
		...
		else if (peer_lan_local != FALSE)
		{
			...
			snprintf(sMnAMPendingMatch.peer_lan_hostport, sizeof(sMnAMPendingMatch.peer_lan_hostport), "%s",
			         peer_lan);
			sMnAMPendingLanBootstrap = TRUE;
			mnVSNetAutomatchAMSetState(MN_AM_BOOTSTRAP_LAN, "defer peer_lan");
			syNetPeerPauseBetweenBootstrapAttempts();
			return;
		}
		else
	...
	{
		mnVSNetAutomatchAMErrEx("reflexive/turn bootstrap failed");
	}
	return;
}
#endif /* !SSB64_NETPLAY_ICE */
...
void mnVSNetAutomatchAMFinalizeVsLoad(void)
{
	...
	mnVSNetAutomatchSetSceneData();
	syTaskmanSetLoadScene();
	/* VS battle still uses the live ICE session; do not call mnVSNetAutomatchAMResetTransport(). */
	mnVSNetAutomatchAMResetSearchState();
}
...
sb32 mnVSNetAutomatchAMConsumeStagingHandshake(void)
{
	...
	if (sMnAMStagingRendezvousStarted == FALSE)
	{
		...
		{
			mnVSNetAutomatchAMErrEx("staging rendezvous failed");
			return FALSE;
		}
...
void mnVSNetAutomatchAMStartSearch(void)
{
	...
	mnVSNetAutomatchAMSetState(MN_AM_ENSURE, "start search");
}
...
void mnVSNetAutomatchMatchmakingTick(void)
{
	...
	if (sMnAMState == MN_AM_POLL)
	{
...
#if defined(SSB64_NETPLAY_ICE)
		mnVSNetAutomatchAMMaybeEnqueueMatchPoll(MN_AM_POLL_MATCH_INTERVAL, FALSE);
#else
		{
			u32 poll_every;
		...
		if ((sMnAMPollPeriodTics % hb_every) == MN_AM_POLL_HEARTBEAT_PHASE)
		{
			...
			if (((sMnAMPollPeriodTics / hb_every) % 4U) == 0U)
			{
				...
				if ((ice_lan != NULL) && (ice_lan[0] != '\0'))
				{
					snprintf(lan_hb, sizeof(lan_hb), "%s", ice_lan);
				}
				else
				{
...
#if defined(SSB64_NETPLAY_ICE)
	if (sMnAMState == MN_AM_BIND)
	{
		char ice_sdp_join[4096];
		const char *lan_for_queue;
		...
		{
			mmIceShutdown();
			mnVSNetAutomatchAMErrEx("ICE gather failed");
		}
		else
#endif
		if (mnVSNetAutomatchAMIceBindTick(ice_sdp_join, (u32)sizeof(ice_sdp_join)) != FALSE)
		{
			char reflex[144];
			const char *ice_lan;
			...
			if (strstr(ice_sdp_join, "a=ice-ufrag:") == NULL)
			{
				u64 now_ms = mnVSNetAutomatchAMNowMs();

				if ((sMnAMBindSrflxLogMs == 0U) || ((now_ms - sMnAMBindSrflxLogMs) >= 2000U))
				{
					port_log("SSB64 Automatch ICE: deferring queue (ice_sdp missing a=ice-ufrag len=%zu)\n",
					         strlen(ice_sdp_join));
					sMnAMBindSrflxLogMs = now_ms;
				}
			}
			else if (mnVSNetAutomatchAMResolveQueueWanEndpoint(reflex, (u32)sizeof(reflex)) != FALSE)
			{
				char turn_ep[144];
				const char *turn_ptr;
				...
				turn_ptr = NULL;
				turn_ep[0] = '\0';
				if (mnVSNetAutomatchAMIceShouldQueueTurnEndpoint() != FALSE)
				{
					if (mmIceGetRelayHostport(turn_ep, sizeof(turn_ep)) != FALSE)
					{
						turn_ptr = turn_ep;
					}
				}
				...
				mmMatchmakingEnqueueJoinQueueIce(FALSE, sMnAMPublicEndpoint, ice_sdp_join,
				                                 ...
				                                 (sMNVSNetAutomatchSlot.is_fighter_selected != FALSE) ? TRUE
				                                                                                      : FALSE,
				                                 lan_for_queue, turn_ptr);
				mnVSNetAutomatchAMSetState(MN_AM_JOIN, "ICE bind complete");
			}
			else if (port_log_debug_active())
			{
				u64 now_ms = mnVSNetAutomatchAMNowMs();

				if ((sMnAMBindSrflxLogMs == 0U) || ((now_ms - sMnAMBindSrflxLogMs) >= 2000U))
				{
					port_log("SSB64 Automatch ICE: waiting for srflx/reflexive WAN endpoint\n");
					sMnAMBindSrflxLogMs = now_ms;
				}
			}
		}
	...
	if (sMnAMState == MN_AM_ICE_CONNECT)
	{
		...
		{
			mnVSNetAutomatchAMMaybeEnqueueMatchPoll(trickle_interval, TRUE);
		}
		if (mnVSNetAutomatchAMPollAbortDuringBootstrap() != FALSE)
		{
			mmIceShutdown();
		...
		else
		{
			...
			ice_tick = mnVSNetAutomatchAMIceConnectTick();
			if (ice_tick < 0)
			{
				const char *why;
...
#if defined(PORT) && defined(SSB64_NETMENU)
	{
		...
		port_watchdog_set_connect_phase_pause(connect_pause != FALSE);
	}
#endif
	if (sMnAMState == MN_AM_ENTER && sMnAMStagingP2PReady != FALSE)
	{
#if defined(SSB64_NETPLAY_ICE)
		if (mmIceIsFailed() != FALSE)
		{
			mmIceShutdown();
			mnVSNetAutomatchAMAbortToCharacterSelect("ICE lost");
		}
		else
#endif
		if (mnVSNetAutomatchAMPollAbortDuringBootstrap() != FALSE)
